...
- (void)setImageURLArray:(NSArray *)imageURLArray {
    ...
    dispatch_async(network_queue, ^{
        for (NSString * imageURL in imageURLArray) {
            ...
            [self.images addObject:image?image:[UIImage new]];
        }
...
- (void)creatSystemPageControll {
    ...
    self.systemPageCtrl.frame = CGRectMake(0, self.height - 30, self.width, 30);
...
//    [self.systemPageCtrl addTarget:self action:@selector(pageControlAction:) forControlEvents:UIControlEventValueChanged];
    //self.systemPageCtrl.backgroundColor = kGrayColor;
    self.systemPageCtrl.pageIndicatorTintColor = [UIColor grayColor]; // 未选中的点的颜色
    
    self.systemPageCtrl.currentPageIndicatorTintColor = kFcolorFontGreen; // 选中的点的颜色
...
- (void)creatCustomPageControl {
    ...
    self.customPageCtrl.frame = CGRectMake(0, self.height - 40, self.width, 40);
...
//        imgViewBtn.frame = CGRectMake(i * self.width, 0, self.width, self.height-50);
...
//        [imgViewBtn setImage:images[i] forState:UIControlStateNormal];
...
//        [imgViewBtn setContentMode:UIViewContentModeScaleAspectFit];
        ...
        UIImageView * imgViewBtn = [[UIImageView alloc] init];
        imgViewBtn.frame = CGRectMake(i * self.width, 10, self.width, self.height-40);
        //        [imgViewBtn setBackgroundImage:images[i] forState:UIControlStateNormal];
        //        [imgViewBtn setBackgroundImage:images[i] forState:UIControlStateHighlighted];
        [imgViewBtn setImage:images[i]];
        imgViewBtn.tag = i + 100;
//        [imgViewBtn addTarget:self action:@selector(imgViewBtnAction:) forControlEvents:UIControlEventTouchUpInside];
        [imgViewBtn setContentMode:UIViewContentModeScaleAspectFit];
