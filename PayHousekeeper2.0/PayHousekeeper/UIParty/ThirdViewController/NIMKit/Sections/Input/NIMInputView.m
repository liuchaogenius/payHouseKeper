...
- (void)initUIComponents
{
    ...
    [_toolBar.inputTextView setPlaceHolder:@"新消息"];
    ...
    _moreContainer = [[NIMInputMoreContainerView alloc] initWithFrame:CGRectMake(0, [NIMUIConfig topInputViewHeight], self.nim_width, [NIMUIConfig bottomMoreInputViewHeight])];
    _moreContainer.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    _moreContainer.hidden   = YES;
    [self addSubview:_moreContainer];
    
    _emoticonContainer = [[NIMInputEmoticonContainerView alloc] initWithFrame:CGRectMake(0, [NIMUIConfig topInputViewHeight], self.nim_width, [NIMUIConfig bottomEmotInputViewHeight])];
...
- (void)inputTextViewToHeight:(CGFloat)toHeight
{
    ...
    toHeight = MIN([NIMUIConfig bottomEmotInputViewHeight], toHeight);
...
- (void)onTouchVoiceBtn:(id)sender {
    ...
    {
        NTESSessionViewController *vc = (NTESSessionViewController *)[nav topViewController];
        if(![vc passVoiceBtn])
        {
            return;
        }
...
- (void)onTouchEmoticonBtn:(id)sender
{
    if (_inputType != InputTypeEmot) {
        ...
        _inputBottomViewHeight = [NIMUIConfig bottomEmotInputViewHeight];
...
- (void)onTouchMoreBtn:(id)sender {
    if (_inputType != InputTypeMedia) {
        ...
        _inputBottomViewHeight = [NIMUIConfig bottomMoreInputViewHeight];
        if ([self.toolBar.inputTextView isFirstResponder]) {
            [self.toolBar.inputTextView resignFirstResponder];
        }
        [UIView animateWithDuration:0.25 animations:^{
