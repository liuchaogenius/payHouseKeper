...
NSInteger NIMButtonItemWidth = 50;
NSInteger NIMButtonItemHeight = 50;
...
{
    NSArray *items = @[[NIMMediaItem item:NTESMediaButtonPicture
             normalImage:[UIImage imageNamed:@"dd_media_picture_normal"]
           selectedImage:nil
                   title:@"图片"],
      
      [NIMMediaItem item:NTESMediaButtonShoot
             normalImage:[UIImage imageNamed:@"dd_media_shoot_normal"]
           selectedImage:nil
                   title:@"拍摄"],
                       
      [NIMMediaItem item:NTESMediaButtonVideoChat
             normalImage:[UIImage imageNamed:@"dd_media_video_chat_normal"]
           selectedImage:nil
                   title:@"视频聊天"]
                       ];
    ...
    [items enumerateObjectsUsingBlock:^(NIMMediaItem *item, NSUInteger idx, BOOL *stop) {
        ...
        if (!shouldHidden) {
            ...
            [btn setTitleColor:RGBCOLOR(157, 160, 164) forState:UIControlStateNormal];
            [btn setTitleEdgeInsets:UIEdgeInsetsMake(70, -60, -10, -10)];
                            ...
                            count:(NSInteger)count
{
    ...
    for (NSInteger btnIndex = 0; btnIndex < count; btnIndex ++)
    {
        ...
        [button addTarget:self action:@selector(onTouchButton:) forControlEvents:UIControlEventTouchUpInside];
        CGRect iconRect = CGRectMake(span + (NIMButtonItemWidth + span) * btnIndex, 15, NIMButtonItemWidth, NIMButtonItemHeight);
