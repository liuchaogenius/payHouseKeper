...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier]) {
        ...
        self.backgroundColor = RGBCOLOR(238, 238, 238);//NIMKit_UIColorFromRGB(0xe4e7ec);
        [self makeComponents];
        [self makeGesture];
    }
    return self;
...
- (void)makeComponents
{
    ...
    _vipImgView = [[UIImageView alloc] initWithFrame:CGRectMake(_headImageView.right-2, _headImageView.bottom-12, 12, 12)];
...
- (void)refresh{
    ...
    if ([self needShowAvatar])
    {
        ...
        NIMKitInfo *info = [[YXManager sharedInstance] getInfoByUserId:self.model.message.from];
        if([info.infoId isEqualToString:[UserInfoData shareUserInfoData].strUserId])
        {
            _vipImgView.hidden = ![UserInfoData shareUserInfoData].isVip;
        }
        else
        {
            FriendsModel *model = [[DDFriendsInfoManager sharedInstance] getFriendInfo:info.infoId];
            _vipImgView.hidden = model?!model.isVip:YES;
        }
    }
...
- (void)layoutAvatar
{
    ...
    if (needShow) {
        ...
        [_vipImgView setX:_headImageView.right-12];
