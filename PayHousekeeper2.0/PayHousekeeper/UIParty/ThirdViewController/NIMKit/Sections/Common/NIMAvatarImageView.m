...
{
    NIMKitInfo *info = [[YXManager sharedInstance] getInfoByUserId:message.from];//[[NIMKit sharedKit] infoByUser:message.from withMessage:message];
    NSURL *url = info.avatarUrlString ? [NSURL URLWithString:info.avatarUrlString] : nil;
//    [self nim_setImageWithURL:url placeholderImage:info.avatarImage];// add by liu
    [self nim_setImageWithURL:url placeholderImage:DEFAULTAVATAR];
}
